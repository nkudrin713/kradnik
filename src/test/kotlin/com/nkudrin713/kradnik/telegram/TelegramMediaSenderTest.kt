...
class TelegramMediaSenderTest {
    ...
    fun sendsVideoWithMetadata(@TempDir tempDir: Path) = runTest {
        ...
        val result = sender.sendVideo(100, file)
    ...
    fun sendsCachedVideo() = runTest {
        ...
        val result = sender.sendCachedVideo(100, "cached-id")

        (request.captured as SendVideo).getParameters()["video"] shouldBe "cached-id"
        result shouldBe TelegramSendResult("video-id", 456)
    }
    ...
    fun sendsAudioWithMetadata(@TempDir tempDir: Path) = runTest {
        ...
        val result = sender.sendAudio(100, file, "title", "artist", 120)
    ...
    fun sendsCachedAudio() = runTest {
        ...
        val result = sender.sendCachedAudio(100, "cached-id")

        (request.captured as SendAudio).getParameters()["audio"] shouldBe "cached-id"
        result shouldBe TelegramSendResult("audio-id", 456)
    }
