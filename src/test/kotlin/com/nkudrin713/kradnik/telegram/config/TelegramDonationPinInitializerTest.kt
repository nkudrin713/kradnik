...
class TelegramDonationPinInitializerTest {
    ...
    fun skipsWhenDisabled() {
        ...
        verify(exactly = 0) { telegramDonationSender.sendPin(any(), any()) }
        verify(exactly = 0) { telegramDonationSender.updatePin(any(), any(), any()) }
    }
    ...
    fun sendsNewPinWhenMessageIdIsMissing() {
        every { telegramDonationSender.sendPin("@mediakradnik", "https://example.com/donate") } returns 123

        initializer(enabled = true).run(args)

        verify { telegramDonationSender.sendPin("@mediakradnik", "https://example.com/donate") }
    }

    @Test
    fun updatesExistingPinWhenMessageIdIsConfigured() {
        every { telegramDonationSender.updatePin("@mediakradnik", 123, "https://example.com/donate") } just runs
        ...
        verify { telegramDonationSender.updatePin("@mediakradnik", 123, "https://example.com/donate") }
    }
